...
;(function ($) {
	...
	var defaults = {
			...
			allCategoryText   : fip.allCategoryText,   // The text for the select all category option
			...
			html:       '<div class="selector">' +
						 ...
						 '<div class="selector-popup" style="display: none;">' + ((this.settings.hasSearch) ?
							 '<div class="selector-search">' +
								 '<input type="text" name="" value="" placeholder="' + fip.placeholder + '" class="icons-search-input"/>' +
	...
	Plugin.prototype = {
		...
		init: function () {
			...
			this.iconPicker.find('.icons-search-input').keyup($.proxy(function (e) {
				...
				this.searchIcon.removeClass('fa fa-search');
				this.searchIcon.addClass('fa fa-times');
			 ...
			 * Quit search
			 */
			this.iconPicker.find('.selector-search').on('click', '.fa.fa-times', $.proxy(function () {
		...
		renderIconContainer: function () {
			...
			if (this.settings.emptyIcon) {
				...
				this.iconContainer.html('<span class="fip-box"><i class="fa fa-minus-circle" data-fip-value="fa fa-minus-circle"></i></span>');

			// If not show an error when no icons are found
			} else if (iconsPaged.length < 1) {
				this.iconContainer.html('<span class="icons-picker-error"><i class="fa fa-minus-circle" data-fip-value="fa fa-minus-circle"></i></span>');
				return;
		...
		setSelectedIcon: function (theIcon) {
			...
			if ( this.settings.useAttribute ) {
				if ( theIcon ) {
					...
					this.iconPicker.find('.selected-icon').html('<i class="fa fa-minus-circle"></i>');
				}
			// Use class
			} else {
				this.iconPicker.find('.selected-icon').html('<i class="' + (theIcon || 'fa fa-minus-circle') + '"></i>');
			}
		...
		toggleIconSelector: function () {
			...
			this.iconPicker.find('.selector-button i').toggleClass('fa fa-caret-up');
			if (this.open) {
		...
		resetSearch: function () {
			...
			this.searchIcon.removeClass('fa fa-times');
			this.searchIcon.addClass('fa fa-search');
